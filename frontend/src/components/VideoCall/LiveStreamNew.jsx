...
import { Mic, MicOff, Video, VideoOff, Share2, Share2Off, Users, Eye, EyeOff } from 'lucide-react';
...
const LiveStreamNew = () => {
  ...
  useEffect(() => {
    const initStream = async () => {
      try {
        ...
        let agoraToken = '';
        try {
          const streamRes = await videoCallAPI.getCallCredentials(streamId);
          channelName = streamRes?.data?.channelName;
          agoraToken = streamRes?.data?.token;
        } catch (e) {
          console.warn('Failed to get stream credentials');
          throw e;
        }
        ...
        // Create Agora client in live mode
  ...
  return (
    <div className="h-screen bg-black text-white flex flex-col">
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-red-900 to-red-700 border-b border-red-800 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-3 h-3 bg-red-500 rounded-full animate-pulse" />
          <span className="text-sm font-bold">LIVE</span>
          <span className="text-xs text-red-200">{formatDuration(streamDuration)}</span>
        </div>
        ...
        {isConnecting && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/80 z-50">
            <div className="text-center">
              ...
              <p className="text-gray-300">Starting live stream...</p>
            </div>
          </div>
        )}
